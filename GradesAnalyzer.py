...
class GradesValidator:
    ...
    def check_grade_range(self, min_g=2, max_g=5):
        ...
        if (grades < min_g).any() or (grades > max_g).any():
            print(f"Бағалар [{min_g}, {max_g}] диапазонында бар")
        else:
            print(f"Барлық бағалар [{min_g}, {max_g}] диапазонында")
    def show_info(self):
        print("Толық ақпарат")
        self.df.info()
        print("Алғашқы 5 жол")
    ...
    def run(self):
        print("11: Валидация fake_grades.csv")
        self.load()
        self.check_types()
        self.check_grade_range()
        self.show_info()
        print("Валидация сәтті жүргізілді")
# 12
class GradesAnalyzer:
    ...
    def load_data(self):
        self.df = pd.read_csv(self.filename)
        print(f"{self.filename} файлын анализ үшін жүктеңіз")
    def compute_subject_means(self):
        if self.df is None:
            raise ValueError("Алдымен деректерді енгізіңіз")
    ...
    def display_results(self):
        if self.subject_means is None:
            print("Деректер жоқ. compute_subject_means() іске қосыңыз")
            return
        print("12-тапсырма: Пәндер бойынша орташа баға (жоғарыдан төменге дейін)")
