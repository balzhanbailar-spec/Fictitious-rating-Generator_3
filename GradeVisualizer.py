...
class GradesValidator:
    ...
    def run(self):
        ...
        if (df['grade'] < 2).any() or (df['grade'] > 5).any():
            print("❌ Обнаружены ошибки в диапазоне оценок!")
        else:
...
class GradeVisualizer:
    ...
    def plot_top_subjects_boxplot(self, n_subjects=5, save_path='grades_boxplot.png'):
        ...
        # ИСПРАВЛЕНИЕ: добавлены hue='subject_id' и legend=False для устранения предупреждения
        sns.boxplot(
            x='subject_id',
            y='grade',
            data=subset,
...
if __name__ == "__main__":
    ...
    print("\n✅ Все задачи выполнены. Скрипт завершен.")